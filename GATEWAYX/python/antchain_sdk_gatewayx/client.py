...
from antchain_sdk_gatewayx import models as _gatewayx_models
...
class Client:
    ...
    def create_ntcloud_gatewayx_bizevent_message(self, request):
        """
        ...
        return self.create_ntcloud_gatewayx_bizevent_message_ex(request, runtime)

    def create_ntcloud_gatewayx_bizevent_message_ex(self, request, runtime):
        """
        Description: 接收业务事件消息，并将相关消息投递给对应的接手方
        Summary: 事件消息创建
        """
        UtilClient.validate_model(request)
        return _gatewayx_models.CreateNtcloudGatewayxBizeventMessageResponse().from_map(self.do_request("1.0", "antcloud.gatewayx.bizevent.message.create", "HTTPS", "POST", "/gateway.do", request.to_map(), runtime))


    def create_ntcloud_gatewayx_file_upload(self, request):
        """
        ...
        return self.create_ntcloud_gatewayx_file_upload_ex(request, runtime)

    def create_ntcloud_gatewayx_file_upload_ex(self, request, runtime):
        """
        Description: 创建HTTP PUT提交的文件上传
        Summary: 文件上传创建
        """
        UtilClient.validate_model(request)
        return _gatewayx_models.CreateNtcloudGatewayxFileUploadResponse().from_map(self.do_request("1.0", "antcloud.gatewayx.file.upload.create", "HTTPS", "POST", "/gateway.do", request.to_map(), runtime))


    def get_ntcloud_gatewayx_file_download(self, request):
        """
        ...
        return self.get_ntcloud_gatewayx_file_download_ex(request, runtime)

    def get_ntcloud_gatewayx_file_download_ex(self, request, runtime):
        """
        Description: 创建HTTP GET获取的文件下载
        Summary: 下载地址获取
        """
        UtilClient.validate_model(request)
        return _gatewayx_models.GetNtcloudGatewayxFileDownloadResponse().from_map(self.do_request("1.0", "antcloud.gatewayx.file.download.get", "HTTPS", "POST", "/gateway.do", request.to_map(), runtime))
