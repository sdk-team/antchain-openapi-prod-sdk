...
export default class Client {
   ...
   * Summary: 检查服务状态
   */
  async statusDemoGatewayCheckEx(request: StatusDemoGatewayCheckRequest, runtime: $Util.RuntimeOptions): Promise<StatusDemoGatewayCheckResponse> {
    Util.validateModel(request);
    return $tea.cast<StatusDemoGatewayCheckResponse>(await this.doRequest("1.0", "demo.gateway.check.status", "HTTPS", "POST", `/gateway.do`, $tea.toMap(request), runtime), new StatusDemoGatewayCheckResponse({}));
  }
   ...
   * Summary: 返回输入值
   */
  async echoDemoGatewayCheckEx(request: EchoDemoGatewayCheckRequest, runtime: $Util.RuntimeOptions): Promise<EchoDemoGatewayCheckResponse> {
    Util.validateModel(request);
    return $tea.cast<EchoDemoGatewayCheckResponse>(await this.doRequest("1.0", "demo.gateway.check.echo", "HTTPS", "POST", `/gateway.do`, $tea.toMap(request), runtime), new EchoDemoGatewayCheckResponse({}));
  }
